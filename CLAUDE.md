...
- `send_card_as_user(via="user")` is **not implemented**. v1.3.9 brute-force probe (see `scripts/explore-card-protobuf.js`) confirmed the cookie-protobuf gateway rejects all `type=14 (CARD)` attempts with the same `richText and card type need for card message` error regardless of field number / wire type / content shape. The validation fires BEFORE Content parsing — likely Feishu requires a different `cmd` for user-identity cards (not cmd=5), OR user-identity card sending is server-side disabled at the cookie auth tier. Workaround: `send_card_as_user(via="bot")` (default) routes through Official API and works. Reverse-engineering deferred to v1.3.10 — needs static analysis of Feishu Web JS bundle, mitmproxy on Feishu Desktop, or upstream LarkAgentX-style protocol dump.

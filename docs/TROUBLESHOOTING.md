...
v1.3.9：事件已经是机器级 SSOT，每条事件全机精确投递一次（v1.4.0 起游标按 profile 独立 `events.cursor.<profile>.json`，`*` / `any` 用全局游标看全部）。老的 per-process 重复问题已修复。
...
- **多个 MCP 进程**：v1.3.9：事件机器级 SSOT，每条事件全机精确投递一次（v1.4.0 起游标按 profile 独立，`*` / `any` 看全部）。老的 per-process 重复问题已修复

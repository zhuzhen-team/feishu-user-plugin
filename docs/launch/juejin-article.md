...
2. **不再是机器人头像：让 Claude Code 以你身份接管整个飞书工作流**
3. **84 工具的飞书 MCP：从读群消息到 OKR 进展，三场景实战拆解**

> 推荐选 1：占满 SEO 关键词（飞书 MCP / Claude Code / 84 工具 / 三层鉴权），强调差异化（"用你本人身份"），数字化（84/3/3）有手感。
...
你也大概见过同事看到这种消息时的反应：直接划走。

机器人发的"周报"没有@真人的存在感。同事不会以为是你写的，更不会觉得这是"该回的消息"。

如果你试过把 Claude Code 接到飞书想做点真正的自动化，那你大概率撞过这堵墙：

- 飞书官方开放 API **没有 `send_as_user` 权限点**
- 哪怕拿到 `user_access_token`（OAuth），发出来的消息一律标记 `sender_type: "app"`
- 群里看到的就是"机器人头像 + 由 [应用名] 发送"

很多场景里这不是 UX 问题，是阻断器。一个我自己踩过的真实例子：让 Claude Code 自动总结群消息发到日报频道，三天后就被组里的产品同学抗议——"这是公告吧，你都不写日报了？" 因为机器人发的消息没人会当成你的工作输出。

为了解决这个问题，过去两个月我做了 [feishu-user-plugin](https://github.com/EthanQC/feishu-user-plugin) —— 一个把**三层鉴权融在一个 MCP server**里的飞书插件，让 Claude Code 既能用机器人能力做苦活（读群、爬文档、批量更新表格），又能用你本人身份做沟通（发消息、@同事、回复 review）。

84 个工具，9 个 MCP prompts，覆盖飞书 IM / 文档 / 多维表格 / 知识库 / 云空间 / 日历 / 任务 v2 / OKR 全套。MIT 协议。

这篇文章不讲架构、不讲 protobuf 反向工程的具体内幕（那是另一篇技术文章），只讲三个实战场景，让你看清这玩意能干什么。

## 一图看懂：三层鉴权解决三类问题

| 鉴权层 | 凭证 | 干什么 |
|---|---|---|
| **用户身份**（cookie + protobuf 反向工程）| `LARK_COOKIE` | 以你本人身份发文本 / 图片 / 文件 / 富文本 / @ / 批量 |
...
Cookie 怎么拿？跟 Claude Code 说一句"帮我设置一下飞书 cookie"——会自动开 Playwright 扫码登录、提取 cookie、写配置。或者手动 DevTools Network 标签复制（不要用 `document.cookie`，拿不到 HttpOnly 那俩）。

接下来三个真实场景。

---

## 场景一：群消息日报（读 + 总结 + 你身份发出）

**痛点**：每天下班前要把"AI 技术解决"群里的讨论梳理一份日报发到 #日报频道。手动做一次 30 分钟，做一周就崩溃。
...
群里其他人看到的是你的头像、你的名字、一条普通消息。没有"由 XX 应用发送"的小尾巴。

**对比飞书 lark-openapi-mcp / 其他机器人 MCP**：他们能完成第 1-4 步，但第 5 步只能机器人发，标识是机器人头像。

**实测耗时**：从我说"开始"到日报发出，平均 12 秒（机型 M2 Pro，网络正常）。

**踩坑提醒**：
- `read_messages` 默认限 50 条；要看完整一天，传 `count: 500` 或者 `since_seconds`
- 外部群 bot 没有 `im:message:readonly` 权限会失败 —— v1.3.7 起自动 fallback 到 UAT 路径，看到 `via: "user"` 字段就是 fallback 成功了
- merge_forward（合并转发）会自动展开，不需要你手动遍历

---

## 场景二：OKR 进展记录（list + 自然语言录入）

**痛点**：周五下班前要给本季度三个 KR 各加一条进展。打开飞书 OKR 页面、点开 KR、选择"添加进展"、写文字、保存——三个 KR 来回点 6 分钟。
...
>
> KR3（学习）：研究了 cookie protobuf 暴力探测方法，写成 docs/COOKIE-PROTOBUF-CAPTURES.md。
...
- 飞书 OKR 开放 API **没有完整 CRUD**——只暴露读 + 进展记录写。本体 create/update/delete 永远不会有，所以"建立 OKR" 这件事插件做不到，但"运营 OKR"已经够用
...
**痛点**：每周组会有 30 分钟讨论，群里同步聊了一堆。会后要把核心决定整理成会议纪要、放到团队知识库的"会议纪要"目录下。
...
| **实时事件** | v1.3.9 机器级 SSOT，全机所有 MCP 进程共享 events.jsonl，每条消息事件全机恰好一次 |
...
简单说：飞书 web 客户端给消息接口的 protobuf payload 里有一些字段我们一开始不知道是什么。v1.3.9 之前 `send_image_as_user` 一直没做出来，因为图片消息的 protobuf 比文本复杂。

后来用了一个**暴力探测**方法：每次发一条带不同字段组合的最小 payload，看哪个组合能在飞书后端通过校验。脚本在 [scripts/explore-image-minimize.js](https://github.com/EthanQC/feishu-user-plugin/blob/main/scripts/explore-image-minimize.js)。

Discovery：图片消息**只需要 `imageKey` + `thumbnailKey` 两个字段**（thumbnailKey 默认等于 imageKey 也通过），width / height / mime / size 飞书后端能自己派生。

同样的方法试了 **CARD 消息（type=14）**，确认服务端**关闭了 cookie 通道发卡片**——所有探测组合都被服务端拒。所以 v1.3.9 把 `send_card_as_user` 重构成纯机器人通道，cookie 通道代码全删了。

### 2. UAT 多进程 race condition

最早期（v1.3.4 之前）有一个怪 bug：6 个 MCP 进程同时跑（Claude Code + Codex + 几个 reload 残留），UAT 过期触发 refresh，6 个进程并发请求 refresh token，**第一个成功**拿到新 UAT，**剩下 5 个**用旧 refresh token 请求被拒，refresh token 直接被飞书拉黑。结果是用户每隔 7 天就要重新 OAuth。

修复：v1.3.5 加了**跨进程文件锁**（`~/.claude/feishu-uat-refresh.lock`，`O_CREAT|O_EXCL`，30s stale）+ 锁持有者**进入临界区前 re-read 一次持久化的 config**——如果发现已有 peer 写入了新 token，直接采用 peer 的，不再发新 refresh 请求。

### 3. v1.3.9 机器级实时事件 SSOT

之前每个 MCP 进程都自己开 WS 监听 + 维护 in-memory event buffer，结果用户被一条消息事件 6 个 MCP 进程通知 6 次。
...
- **官网（中文优先）**：https://ethanqc.github.io/feishu-user-plugin/
...
- 本项目**仅用于个人与企业内部用途，不是商业 SaaS 产品**
- Cookie + protobuf 反向工程层未经飞书官方背书，请遵守你所在企业的 IT 政策
- 官方 API 层完全使用飞书公开开放 API，需要在飞书开放平台创建自建应用并申请相应权限
- 公开商业部署 / 多租户 SaaS 场景请先评估法律和合规风险

## 致谢

- [@cv-cat](https://github.com/cv-cat) 的 LarkAgentX / OpenFeiShuApis 项目，最早期的飞书协议反向工程参照
- [Model Context Protocol](https://modelcontextprotocol.io) —— Anthropic / PulseMCP / GitHub / Stacklok 共维的 MCP 标准
- [Anthropic Claude Code](https://claude.com/claude-code) —— 这个仓库 80% 的代码由 Claude Code 协同完成

如果你试用碰到问题，[issues 区](https://github.com/EthanQC/feishu-user-plugin/issues) 或 [discussions](https://github.com/EthanQC/feishu-user-plugin/discussions)（v1.3.9 起开了）见。

---

**封面图建议**：1080x540 黑底白字，主标 "feishu-user-plugin / 飞书 MCP 服务器"，副标 "84 工具 · 3 层鉴权 · 以你身份发消息"，底部 "Claude Code · Codex · Cursor · Windsurf"。

...
- **Cursor** 的 MCP support 是 2025 Q4 之后新功能，client-side prompt UI 尚在迭代；安装本插件后 85 工具应能在 Cursor 调用，prompt 形式建议直接给 Cursor 发自然语言 instead of `/prompt` slash
...
每个 PR 跑 `node scripts/smoke.js diff`：spawn `src/index.js` 作 stdio child → 发 `initialize` / `tools/list` / `tools/call get_login_status` / `prompts/list` → 校验 85 tools / 9 prompts + login_status response shape 不变。等价于在 Claude Code / Codex 协议层做 smoke，但只 verify 服务端协议正确性，不涉及客户端 UI。
